...
const AdministratorDashboard = () => {
  ...
  const activityStats = [
    {
      ...
      value: data.validatedUsers ?? 0,
      text: "verified accounts",
    },
    {
      label: "Unvalidated Users",
      value: data.invalidatedUsers ?? 0,
      text: "pending verification",
    },
    {
      label: "Active Patients",
      value: data.activePatients ?? 0,
      text: "submitted in last 30 days",
    },
    {
      label: "Inactive Patients",
      value: data.inactivePatients ?? 0,
      text: "no recent submission",
    },
    {
      label: "Failed Logins last 24 hours",
      value: data.failedLoginAttemptsLastDay ?? 0,
  ...
  const reportStats = [
    { label: "Total Reports", value: data.totalReports ?? 0, text: "all time" },
    {
      ...
      value: data.reportsLastDay ?? 0,
      text: "last 24 hours",
    },
    {
      label: "Latest Reports",
      value: data.reportsLast7Days ?? 0,
      text: "last 7 days",
    },
    {
      label: "Latest Reports",
  ...
  const riskStats = [
    {
      label: "Average Stroke Risk",
      value: data.averageRiskStroke ?? 0,
      text: "across all patients",
    },
    {
      label: "Average CVD Risk",
      value: data.averageRiskCVD ?? 0,
      text: "across all patients",
    },
    {
      label: "Average Diabetes Risk",
      value: data.averageRiskDiabetes ?? 0,
      text: "across all patients",
    },
  ];
  ...
  return (
    ...
    >
      ...
      <Box sx={{ display: "flex", gap: 2, mb: 3, mt: 1 }}>
        {userStats.map(({ label, value, text }) => (
          <Card key={label} sx={{ borderRadius: "10px", flex: 1 }}>
            <CardContent>
              <Typography gutterBottom sx={{ color: "#747474", fontSize: 12 }}>
                {label.toUpperCase()}
              </Typography>
      ...
      {/* Report stats */}
      <Typography sx={{ color: "#747474", letterSpacing: "0.05em" }}>
        REPORTS
      </Typography>
      <Box sx={{ display: "flex", gap: 2, mb: 3, mt: 1 }}>
