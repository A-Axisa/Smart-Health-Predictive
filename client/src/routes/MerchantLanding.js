...
const MerchantLanding = () => {
  ...
  const stats = [
    { label: "Total Patients", value: data.totalPatients ?? 0, text: "under your care" },
    { label: "Total Reports", value: data.totalReports ?? 0, text: "all time" },
    { label: "Reports (30 days)", value: data.reportsLast30Days ?? 0, text: "total submissions" },
    { label: "Inactive Patients", value: data.inactivePatients ?? 0, text: "no submissions in 30 days" },
  ];
          ...
          {["stroke", "cvd", "diabetes"].map((condition) => {
            ...
            return (
              <Box key={condition} sx={{ mb: 2.5 }}>
                ...
                {[["Low", "#3aa82c", low], ["Moderate", "#ffa113", mod], ["High", "#da2828", high]].map(([label, color, val]) => (
                  <Box key={label} sx={{ display: "flex", alignItems: "center", gap: 2, mt: 1 }}>
                    <Typography sx={{ color: "#747474", width: 60 }}>
                      {label}
                    </Typography>
                    <Box sx={{ flex: 1, height: 8, bgcolor: "#f0f0f0", borderRadius: 2, overflow: "hidden" }}>
                      <Box sx={{ height: "100%", width: `${Math.round(val / maxVal * 100)}%`, bgcolor: color }} />
                    </Box>
                    <Typography sx={{ color: "#747474" , width: 20, textAlign: "right"}}>
                      {val}
                    </Typography>
                  </Box>
                ))}
          ...
          <Box sx={{ maxHeight: 380, overflowY: "auto" }}>
            {(data.reportActivity ?? []).length === 0 ? (
              ...
              (data.reportActivity).map((a) => (
                <Box key={a} sx={{ display: "flex", alignItems: "center", gap: 1, py: 1, borderBottom: "1px solid #e0e0e0" }}>
