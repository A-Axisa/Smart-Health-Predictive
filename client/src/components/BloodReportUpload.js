...
const BloodReportUpload = ({ onChange }) => {
  ...
  function extractPatientInfoAsDict(text) {
    ...
    let HbA1cLevel = Number(
      formattedText[formattedText.indexOf(HbA1cLabel) + HbA1cPercentageOffset],
    );
    let aveBloodGlucoseLevel = convertHbA1cToAverageBGL(HbA1cLevel);
    let cholesterolLevel = Number(
      formattedText[
        formattedText.indexOf(cholesterolLabel) + cholesterolOffset
      ],
    );
