...
const PasswordInputField = ({
  ...
  async function updateState(e) {
    ...
    const output_password = truncate
      ? inputPassword.substring(0, MAX_LENGTH - 1)
      : inputPassword;
    onChange?.({
  ...
  function isPasswordValid(inputPassword) {
    ...
    if (restrictLength) {
      isValidPassword =
        inputPassword.length > MIN_LENGTH && inputPassword.length < MAX_LENGTH;
    }

    // Confirm the input contains each required characters types.
    if (requireCharacters) {
      if (!UPPERCASE_REGEX.test(inputPassword)) {
        return false;
      }
      if (!LOWERCASE_REGEX.test(inputPassword)) {
        return false;
      }
      if (!NUMERICAL_REGEX.test(inputPassword)) {
        return false;
      }
      if (!hasCommonCharacter(VALID_SYMBOLS, inputPassword)) {
        return false;
      }
    }
  ...
  function displayErrorText() {
    ...
    if (restrictLength) {
      if (password.length <= MIN_LENGTH) {
        length_warning += "be longer than " + MIN_LENGTH + " characters";
      } else if (password.length >= MAX_LENGTH)
        length_warning += "be shorter than " + MAX_LENGTH + " characters";
    }
