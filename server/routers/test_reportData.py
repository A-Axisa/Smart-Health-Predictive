...
def test_get_data(setup_once_for_all_tests):
    ...
    db_conn = next(get_db())
    healthDataID = db_conn.query(HealthData.HealthDataID).order_by(HealthData.HealthDataID.desc()).first()[0]
...
def test_delete_data():
    db_conn = next(get_db())
    healthData = db_conn.query(HealthData).order_by(HealthData.HealthDataID.desc()).first()
    response = client.delete(f"/reportData/{healthData.HealthDataID}")
    assert response.status_code == 200
    assert response.json() == {"message": "Health report data successfully deleted"}
